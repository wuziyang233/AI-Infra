...
你的读者是关注 AI Infra、算力平台、模型服务、Agent 平台和企业 AI 产品机会的 B 端产品/技术负责人。日报必须像决策简报，不要像新闻列表。
...
用一句话概括今天最重要的 AI Infra 趋势。必须指出“为什么今天重要”。
...
只使用输入文章中 `区域: 国内`、`语言: zh` 或中文来源的内容。格式同上。如无内容写"本日暂无相关情报"，不要用海外内容硬凑。

## 海外动态

只使用输入文章中 `区域: 海外` 或英文来源的内容。格式同上。

## 产品机会

基于当日报文提炼 3-5 条产品机会，每条必须包括：
- 机会名称
- 目标用户
- 为什么现在值得做
- 可落地的产品形态
...
提炼 3-5 条风险提示，每条必须说明影响对象和应对建议，并带来源。

## 规则
1. 严格使用 Markdown，每个观点块后必须有 `来源`，列出支撑链接
2. 多个文章支撑同一结论时，合并成一个观点，并列多个来源
3. 没有来源的信息不要写进日报；不要编造链接、公司动作或产品名称
4. 优先提炼“趋势/变化/影响”，不要逐篇复述
5. 国内动态和海外动态必须按输入元数据分区，不要混放
6. 如果国内内容不足，明确写"本日暂无相关情报"，不要用海外新闻填充
7. 直接输出 Markdown，不要包含代码块标记
8. 全文使用中文"""
...
def generate_report(date: str | None = None) -> int | None:
    ...
    rows = db.execute(
        """SELECT a.id, a.title, a.url,
                  ...
                  a.content, a.relevance, a.published_at, a.collected_at,
                  COALESCE(s.name, '未知来源') as source_name,
                  COALESCE(s.category, 'overseas_ai') as source_category,
                  COALESCE(s.language, 'en') as source_language
           FROM articles a
           LEFT JOIN sources s ON a.source_id = s.id
           WHERE date(a.collected_at) = ?
             AND (a.summary IS NOT NULL OR a.content IS NOT NULL)
           ORDER BY
             CASE WHEN COALESCE(s.language, 'en') = 'zh' THEN 0 ELSE 1 END,
             a.relevance DESC,
             a.id DESC""",
        (report_date,),
    ).fetchall()
    ...
    if not rows:
        ...
        rows = db.execute(
            """SELECT id, title, url,
                      ...
                      content, relevance, published_at, collected_at,
                      '未知来源' as source_name,
                      'overseas_ai' as source_category,
    ...
    for r in rows:
        ...
        body = (r["summary"] or "")[:650]
        region = _region_label(r["source_category"], r["source_language"])
        articles_text.append(
            f"**{r['title']}**\n"
            f"来源:{r['source_name']} | 分类:{r['source_category']} | 语言:{r['source_language']} | 区域:{region} | 相关度:{r['relevance']:.2f}\n"
            f"发布时间:{r['published_at'] or '未知'} | 采集时间:{r['collected_at']}\n"
            f"摘要:{body}\n"
            f"洞察:{ins_text or '无'}\n"
            f"链接:{r['url']}\n"
        )
...
def _region_label(category: str, language: str) -> str:
    if language == "zh" or category in {"china_ai", "wechat"}:
        return "国内"
    return "海外"
